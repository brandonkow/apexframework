...
The 97 property/profile controls are divided into 18 checkpoints. The six conversation preferences remain in the existing workspace; they do not count as investment criteria. Browser progress is not cross-device account storage. Export before clearing browser data or changing devices. Existing server-side reports and account memory continue to use the configured backend storage.
