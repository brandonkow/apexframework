...
The owner has deferred LLM integration for now. Continue the framework-only build without provider keys. Existing opt-in provider support is retained, but unavailable AI controls are hidden and model quality testing is deferred, not a blocker for this path. This does not claim human-like open-ended reasoning or automatic model training.
...
The owner interface first previews additions, changes, unchanged records, removals and current versus excluded coverage. Nothing is published until confirmation. `POST /api/owner/discovery` accepts `{action:"preview", catalogue}` and `{action:"publish", catalogue, previewToken}`. The token binds the normalized file to the existing source snapshot; a source change or modified payload requires another preview. Concurrent unrelated writes are preserved. Authenticated legacy direct-import API clients remain supported. IDs that collide after trimming or truncation are rejected.
...
`GET /api/assistant/status` reports coverage, LLM availability, account state and storage durability. `GET/POST /api/assistant/cases` and case-scoped `message`, `confirm`, `step`, `cancel`, `select`, `property`, `stage`, `task`, `evidence`, `outcome` and `export` operations manage investigations. Changes require a matching case revision. A user cannot choose another user's scope or send forged ranked candidates. Candidate selection rechecks current publication and freshness.
...
Assistant / More / Restore a backup accepts an `apex-investigation.v1` JSON export up to 2 MB. It is available even when no cases exist. Preview sends the file only to Apex for validation; it creates no case and never calls a model. Confirmation creates a separate private case in the current guest/account scope, never overwriting an existing case. Repeating the same backup in that scope opens its existing recovered case, including after a restart. Existing case-count and storage limits apply.

Working property and financial inputs return as user-declared assumptions. Income-basis confirmation, journey evidence gates and DCF verification flags need review again. The declared ownership stage is retained, with its standard tasks open. The search is not restarted. Historical conversation, action notes, outcomes, original working evidence, source snapshots, thesis records and file notes remain in an inert, labelled archive; they are not used in current calculations, model context or sharing approvals. Prior custom actions and contractual dates are reference history, not reactivated deadlines. Original file bytes, storage pointers, account access and source publication authority are not restored. A previously catalogue-selected property becomes an unverified private property, not a refreshed listing. Reattach original files separately when private storage is available.

Export first flushes pending tool edits and requests the saved server version. A sync conflict blocks the export rather than silently omitting edits; resolve it or use the separate unsaved-input export. An inline private-download link remains available if the automatic download is blocked, and is cleared on investigation changes or saved tool edits. Unsubmitted forms are not saved by Export. Backups contain private financial/context records: keep them private. Recovery is a manual safeguard, not a substitute for durable production storage.

The additive PostgreSQL migration stores assistant state in `estatelab_core.assistant`. JSON remains a local fallback. Vercel's local JSON is explicitly temporary, not durable production storage. Configure `DATABASE_URL` before relying on production accounts. The owner selected a separate Apex database; private-table RLS and Data API grant protection are documented in `POSTGRESQL.md`. Local embedded-PostgreSQL role tests are not proof that the hosted database is connected or configured. AI brief extraction requires explicit opt-in, is schema-validated and needs a configured provider key. Failure falls back transparently to basic extraction and editable confirmation.
...
Selected-case replies now run the existing decision engine on the current working inputs, including older selected cases without a separate working copy. Generic decision questions use its actual summary, blocker, counter-thesis and next action. Hard stops appear before relevant financial, rental or workflow guidance; they are labelled as rule triggers from user-declared inputs, not verified facts. Only the first two are shown in chat, with an explicit count when more require review in the tools. Post-purchase stops request holding review rather than implying that the purchase has not happened or automatically instructing a sale.

While a hard stop or changed/unavailable published-source warning is active, substantive replies stay in framework mode and no model call is made. This deliberately prevents optional AI wording from overriding the stop; it is not a claim that the model has reviewed the case. Correcting saved assumptions triggers a fresh engine assessment on the next message. Merely asking to ignore the rules or supplying a forged assessment cannot clear it. A correction removes a rule trigger only; it does not independently verify the corrected claim. Greetings remain short and do not repeat investment warnings. Missing-evidence cases still require verification, even when AI reasoning is available.
...
## Review a privately supplied property

`Review a property` opens one inline form with two required fields: property/project name and micro-area. Asking price and a public HTTPS reference link are optional. Opening or cancelling the untouched form does not create an investigation. Submitting starts or updates a private case; a rejected submission can leave the newly created blank case, while the form retains its values for correction. The chat and financial-entry controls step aside while this form is open. The unsubmitted browser draft is not durable memory.

`POST /api/assistant/cases/:id/property` takes the matching case `revision` and a `property` object containing only `projectName`, `area`, optional `askingPrice` and optional `sourceUrl`. It creates a private `user_supplied` snapshot, never an owner-published listing or verified market observation. Missing price, rent and availability stay unknown. Reference links are recorded but not opened or fetched. Starting this review requires neither a catalogue feed nor an LLM call.

Property-specific working inputs, checkpoint evidence and DCF assumptions restart from the supplied identity and optional price. The separately saved financial profile and its declared basis survive. Existing conversation, private observations, files and completed search history remain in the same investigation; review their relevance rather than treating them as proof about the newly supplied property. The selected snapshot remains immutable; later tool edits change only the private working copy. Another property requires a new investigation. Active financial intake and running searches must finish or be cancelled first.

The site-visit action plan begins with every check open. Selection does not claim that a visit occurred, prove ownership, approve financing or recommend buying. Reasoning explicitly distinguishes this unverified starting point from catalogue evidence and continues to request completed sales, achieved rent, building checks and professional verification. Existing private export, deletion, ownership planning and thesis-review boundaries also apply. JSON on a serverless deployment remains temporary; this path does not remove the durable-storage requirement for reliable long-term memory.
...
- A user can privately investigate a property they already found through `Review a property`, with only name and area required. This does not add catalogue coverage or independently verify the property.
...
`tests/assistant-property.test.js` covers private identity, unknown price, immutable source versus editable working inputs, finance preservation, unsafe URLs, forged verification claims, active-search/intake conflicts, account isolation, stale revisions, export/delete and zero provider/catalogue writes. An isolated browser audit of the private-entry flow checked cancel/error recovery, name-and-area-only submission, saved tool figures in framework replies, reload, financial-intake handoff and draft clearing on case switching. Both the form and selected investigation were checked for overflow at 320, 390, 768 and 1440 CSS pixels; the form actions meet a 44-pixel minimum height. Synthetic test properties were not published to production.

`tests/assistant-decision-boundary.test.js` uses the actual engine to test management rejection, investor-capacity pauses, missing evidence, corrected inputs, legacy snapshots, changed-source boundaries, greeting behavior, post-purchase wording and provider-call prevention. The isolated HTTP lifecycle test also saves a management concern through the working-context endpoint, verifies the resulting rejection survives server restart, and confirms that a corrected input is reassessed instead of repeating an old warning from chat history. These checks do not substitute for live configured-model quality testing.

`tests/assistant-catalogue.test.js` and `tests/assistant-recovery.test.js` cover preview-only behavior, normalized-ID collisions, stale confirmation, concurrent writes, private scope, duplicate restores, malformed/oversized backups and inert historical authority. The actual HTTP lifecycle also verifies source preview/publish and backup recovery, separate guest access and idempotency after restart. No real source records or user backups are sent to production by these tests.

The 2026-09-20 recovery/import acceptance pass ran on isolated local storage with no provider keys: all 192 tests passed. Browser checks covered empty-case recovery, explicit preview/confirmation, escaped readable history, reopened checks, unverified DCF comparables, restored tool values, saved edits, duplicate restore, reload, case-switch cleanup, catalogue preview/publication and token-change invalidation. Layout checks at 320, 390, 768 and 1440 CSS pixels found no horizontal overflow; recovery and publication actions meet a 44-pixel touch target. The prepared backup link was visible, but the embedded browser did not expose a download event, so a downloaded file from that browser was not verified. Export JSON and persistence were checked through the HTTP lifecycle. No production data was imported, and these local checks do not establish hosted database durability.
...
| Framework calculations, contrary case and a small explained shortlist | Existing engine reused; scenario and browser tests; model integration and live model quality deferred by owner |
...
| Private, continuing property record with editable assumptions | Versioned working-context API, end-to-end sync and embedded-PostgreSQL role protection tests; separate production database setup, hosted access verification and durability checks still required |
| Collect private documents/photos with permission | Case-scoped originals, bounded local extraction and reviewed notes delivered; local API/browser tests; production storage required, optional AI reading deferred |
