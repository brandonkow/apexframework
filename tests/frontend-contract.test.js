...
test("frontend selectors and stylesheet structure stay valid", async () => {
  ...
  assert.match(html, /id="ownerIntelControls"[\s\S]*?data-owner-intel-filter="missing"[\s\S]*?id="ownerIntelCopyBrief"[\s\S]*?id="ownerIntelExport"[\s\S]*?id="ownerIntelImport"[\s\S]*?id="ownerIntelRestoreHistory"[\s\S]*?id="ownerIntelRestorePhrase"[\s\S]*?id="ownerIntelRestoreConfirm"[\s\S]*?id="ownerIntelRestoreLog"/, "Owner intelligence needs coverage filters, backup export, typed restore, and restore history actions.");
