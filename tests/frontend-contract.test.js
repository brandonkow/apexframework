...
test("frontend selectors and stylesheet structure stay valid", async () => {
  ...
  assert.match(html, /id="ownerIntelControls"[\s\S]*?data-owner-intel-filter="missing"[\s\S]*?id="ownerIntelCopyBrief"[\s\S]*?id="ownerIntelExport"[\s\S]*?id="ownerIntelImport"[\s\S]*?id="ownerIntelRestoreHistory"[\s\S]*?id="ownerIntelBackupReminder"[\s\S]*?id="ownerIntelRestorePhrase"[\s\S]*?id="ownerIntelRestoreConfirm"[\s\S]*?id="ownerIntelRestoreLog"/, "Owner intelligence needs coverage filters, backup export, reminder, typed restore, and restore history actions.");
  ...
  assert.match(app, /function loadOwnerIntelligence[\s\S]*?\/api\/owner\/market\/projects[\s\S]*?\/api\/owner\/development-cases[\s\S]*?\/api\/owner\/documents[\s\S]*?\/api\/owner\/restore\/history/, "Owner intelligence must unify projects, cases, evidence, and owner data safety status.");
