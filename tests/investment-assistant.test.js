...
test("private discovery lifecycle persists, resumes and rejects cross-user edits", { timeout: 60000 }, async t => {
  ...
  const context = structuredClone(item.toolContext);
  context.dealCard.managementQuality = "Weak";
  assert.equal((await action("context", { contextRevision: item.working.revision, context })).status, 200);
  await action("message", { message: "Should I proceed with the purchase?", allowAi: true });
  assert.equal(item.messages.at(-1).mode, "framework");
  assert.match(item.messages.at(-1).content, /Framework boundary: reject.*Weak management/s);
  await stop(); await start();
  item = (await request(`/api/assistant/cases/${id}`, null, { cookie: guest })).data.case;
  assert.match(item.messages.at(-1).content, /Weak management/);
  context.dealCard.managementQuality = "Strong";
  await action("context", { contextRevision: item.working.revision, context });
  await action("message", { message: "Explain the updated decision" });
  assert.doesNotMatch(item.messages.at(-1).content, /Weak management/, "The current answer must not reuse a resolved old trigger from conversation history.");
